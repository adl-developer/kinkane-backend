...
export const commercePaths = {
  ...
  '/api/v1/orders/track': {
    post: {
      tags: [ORDERS],
      ...publicEndpoint,
      summary: 'Track an order with the order number',
      description: [
        'The "Track My Order" call: the order number printed on the confirmation — `ORD-7K2M9QX4` — plus the email address the order was placed with. Those two strings are all a customer ever needs.',
        '',
        'Works for guests **and** signed-in customers, and is not scoped to the caller — someone reading an order number off a printed slip should not be told "not found" because the order hangs off another account.',
        '',
        '### Why the email is required',
        'The order number is an identifier, not a credential. It is printed on receipts and pasted into support tickets, so the email is the second factor. Do not build a UI that submits the order number alone.',
        '',
        'The order number is case-insensitive, the `ORD-` prefix is optional, and spaces and dashes are stripped — accept whatever the customer types.',
        '',
      ...
      requestBody: body(object({
        reference: {
          type: 'string',
          description: 'The order number. Case-insensitive; the `ORD-` prefix is optional and spaces and dashes are ignored.',
          example: 'ORD-7K2M9QX4',
        },
        email: {
          type: 'string',
      ...
      responses: {
        200: json('The order, with its lines.', ref('Order')),
        400: resp('ValidationError'),
        404: json('No such order number, or the email does not match. Deliberately indistinguishable.', ref('Error'),
          { error: 'Order not found' }),
        429: resp('RateLimited'),
      },
    },
