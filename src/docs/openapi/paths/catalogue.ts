...
const listFilterParams = [
  ...
  param('priceMin', 'query', { type: 'number', minimum: 0 },
    'Lowest price, inclusive, in pounds — `10` means £10, not 1000 pence. **Requires `shoppable=true`**; the price lives on the supplier row only that path consults, and sending it without the flag is a 400 rather than a silently unfiltered page.'),
  param('priceMax', 'query', { type: 'number', minimum: 0 },
    'Highest price, inclusive, in pounds. Same rules as `priceMin`.'),
  param('currency', 'query', { type: 'string', minLength: 3, maxLength: 3 },
    'Ignored. The shop sells in GBP only, exactly as Gardners prices it; kept so older clients do not break.', { example: 'GBP' }),
